...
@Namespace("/pages/jsp/admin")
@Action("actionadmin")  //注解定义Action
@InterceptorRef("adminStack")  //引入数据验证拦截器 
@Results(value={
		         @Result(name="updatepassword.error",location="/pages/jsp/admin/admin/admin_password_edit.jsp"),//修改密码数据验证失败返回路径
		         @Result(name="update.error" ,location="/pages/jsp/admin/admin/admin_admin_update.jsp"), //用户信息修改数据验证失败 返回路径
		         @Result(name="udpdatepawpre",location="/pages/jsp/admin/admin/admin_password_edit.jsp"),
		         @Result(name="member.update",location="/pages/jsp/admin/admin/admin_admin_update.jsp"),
		         @Result(name="input" ,location="/pages/jsp/admin/admin/admin_admin_update.jsp")
                 }) 
public class MemberActionAdmin extends AbstractAction {
	...
	public String updatePre(){
		//1.取得用户
		Member m =(Member) super.getRequest().getSession().getAttribute("admin");
	...
	public String update(){
		...
		try {
			if(this.iMemberServiceCommon.update(this.user)){
				...
				super.getRequest().getSession().setAttribute("admin", m);
				super.setUrlAndMessage("admin.update.pages", "update.success");
			}else{
	...
	public String updatepassword(){
		...
		try {
			...
			if(flag){
				...
				super.setUrlAndMessage("admin.index.pages", "updatepassword.defeat");
			}
		} catch (Exception e) {
			super.setUrlAndMessage("admin.index.pages", "updatepassword.defeat");
			e.printStackTrace();
