...
@Repository
@ParentPackage("root")
@Namespace("/pages/jsp/emp")
@Action("actionemp")
@InterceptorRef("empStack")  //引入数据验证拦截器
@Results(value={@Result(name="updatepassword.error",location="/pages/jsp/emp/emp/emp_password_edit.jsp"),//数据验证失败返回路径
	            @Result(name="udpdatepawpre",location="/pages/jsp/emp/emp/emp_password_edit.jsp"),
	            @Result(name="member.update",location="/pages/jsp/emp/emp/emp_emp_update.jsp")}) 
public class MemberActionEmp extends AbstractAction {
	...
	public String updatepassword(){
		...
		//取出用户id
		Member m =(Member)super.getRequest().getSession().getAttribute("member");
		try {
			boolean flag=this.iMemberServiceCommon.updatepassword(m.getMid(), new MD5Code().getMD5ofStr(this.oldpassword), new MD5Code().getMD5ofStr(this.newpassword));
			if(flag){
				super.setUrlAndMessage("login.page", "updatepassword.success");
			}else{
				super.setUrlAndMessage("member.index.pages", "updatepassword.defeat");
			}
		} catch (Exception e) {
			super.setUrlAndMessage("member.index.pages", "updatepassword.defeat");
			e.printStackTrace();
	...
	public String update(){
		...
		try {
			if(this.iMemberServiceCommon.update(this.user)){
				...
				super.getRequest().getSession().setAttribute("member", m);
				super.setUrlAndMessage("member.update.pages", "update.success");
			}else{
				super.setUrlAndMessage("admin.update.pages", "update.defeat");
			}
		} catch (Exception e) {
			// TODO Auto-generated catch block
			super.setUrlAndMessage("member.update.pages", "update.defeat");
			e.printStackTrace();
